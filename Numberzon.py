...
def del_ligne_file(number):

    print(number.strip()+" non valide suppression")
    with open("phone_numbers.txt","r+") as f:
        new_f = f.readlines()
        f.seek(0)
        for line in new_f:
            if number.strip() not in line:
                f.write(line)
        f.truncate()
...
def test(number):
    ...
    data="prevRID=" + urllib.parse.quote(prevrid_search)
    ...
    if "incorrect" in response.text:
        ...
        del_ligne_file(number)
    elif "survenu" in response.text:
        print('message survenu')
        del_ligne_file(number)

        
filin=open ("phone_numbers.txt","r")
